...
class TikTokClient:
    ...
    async def search_tiktok_advertisers(
        self, *, query: str, region: str = "DE", count: int = 10
    ) -> dict[str, Any]:
        ...
        params = {
            k: v
            for k, v in {"query": query, "region": region, "count": count}.items()
            if v is not None
        }
        return await self._client.get("/v1/tiktok/ads/advertisers", params=params)

    async def get_tiktok_ad_detail(self, ad_id: str, *, region: str = "DE") -> dict[str, Any]:
    ...
    async def tiktok_shop_deals_feed(self, deal: str, *, region: str = "US") -> dict[str, Any]:
        """TikTok Shop deals feed.

        Generated from the OpenAPI spec; returns the raw response dict.
        """
        params = {k: v for k, v in {"region": region}.items() if v is not None}
        return await self._client.get(f"/v1/tiktok/shop/deals/{deal}", params=params)
    ...
    async def tiktok_shop_best_sellers(
        self, *, region: str = "US", count: int = 20
    ) -> dict[str, Any]:
        """TikTok Shop best sellers.

        Generated from the OpenAPI spec; returns the raw response dict.
        """
        params = {k: v for k, v in {"region": region, "count": count}.items() if v is not None}
        return await self._client.get("/v1/tiktok/shop/ranking", params=params)
